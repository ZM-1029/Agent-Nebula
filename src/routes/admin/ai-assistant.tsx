...
export const Route = createFileRoute("/admin/ai-assistant")({
  head: () => ({
    meta: [
      { title: "AI Assistant — Helix" },
      {
        name: "description",
        content: "AI suggested replies, sentiment, auto-categorization and bot monitoring.",
      },
    ],
  }),
  component: AIPage,
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Augment agents with smart replies, sentiment and auto-routing.
          </p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        <GlassCard>
          ...
          <p className="text-[11px] text-muted-foreground">
            Live drafts for the chats your team is working on.
          </p>
          <div className="mt-3 space-y-3">
            {[
              {
                c: "Eva Brooks",
                t: "Upgrade synced. Want me to send a confirmation email to your billing contact as well?",
              },
              {
                c: "Marcus Field",
                t: "You're hitting the 1000 req/min plan limit. I can bump you to Pro+ for $80/mo if helpful.",
              },
              {
                c: "Hana Park",
                t: "Re-running the September export now — you'll get an email when the file is ready.",
              },
              {
                c: "Theo Adler",
                t: "Looping in our pricing manager Priya. She'll be in this thread within 30 minutes.",
              },
            ].map((s, i) => (
        ...
        <div className="space-y-4">
          <GlassCard>
            <p className="text-sm font-semibold flex items-center gap-2">
              <Brain className="h-4 w-4 text-accent-violet" /> Sentiment mix
            </p>
          ...
          <GlassCard>
            <p className="text-sm font-semibold flex items-center gap-2">
              <Bot className="h-4 w-4 text-accent-blue" /> Bot monitor
            </p>
            <div className="mt-3 grid grid-cols-3 gap-2 text-center">
              {[
                ...
                <div key={s.l} className="rounded-xl bg-background/40 p-2">
          ...
          <GlassCard>
            <p className="text-sm font-semibold flex items-center gap-2">
              <BookOpen className="h-4 w-4 text-accent-amber" /> Knowledge recs
            </p>
            <ul className="mt-2 space-y-1.5 text-xs">
              {[
                "Add article: 'Why webhook retries can take 30 minutes'",
                "Update SSO troubleshooting page (847 views, 18% bounce)",
                "Translate billing FAQ into German (3 weekly requests)",
              ].map((r, i) => (
