...
import { roles, permissionMatrix } from "@/lib/admin-mock/data";
import { Check, Minus, ShieldCheck, Plus } from "lucide-react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

export const Route = createFileRoute("/admin/roles")({
  head: () => ({ meta: [{ title: "Roles & Permissions — Helix" }, { name: "description", content: "Granular roles, permission matrix and access controls." }] }),
  component: RolesPage,
});
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary" /> Roles & Permissions</h1>
          <p className="text-sm text-muted-foreground">Granular role-based access for your workspace.</p>
        </div>
        <Button className="gradient-primary text-primary-foreground"><Plus className="mr-1.5 h-4 w-4" /> New role</Button>
      </div>

      <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-5">
        {roles.map(r => (
          <GlassCard key={r.id} interactive className="relative overflow-hidden">
            <div className={cn("absolute -right-6 -top-6 h-24 w-24 rounded-full bg-gradient-to-br blur-2xl opacity-60", toneClass[r.color])} />
            <div className="relative">
              <p className="text-sm font-semibold">{r.name}</p>
              <p className="mt-1 text-[11px] text-muted-foreground">{r.desc}</p>
              <p className="mt-4 text-2xl font-semibold">{r.count}<span className="ml-1 text-xs font-normal text-muted-foreground">members</span></p>
            </div>
          </GlassCard>
        ))}
      </div>
      ...
      <GlassCard className="p-0">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border/60 text-left text-[11px] uppercase text-muted-foreground">
              ...
              {permissionMatrix.map((row,i) => (
                <tr key={i} className="border-b border-border/40">
                  ...
                  {(["super","mgr","agent","analyst","viewer"] as const).map(k => (
                    <td key={k} className="p-3 text-center">{row[k] ? <Check className="mx-auto h-4 w-4 text-primary" /> : <Minus className="mx-auto h-4 w-4 text-muted-foreground/50" />}</td>
                  ))}
                </tr>
              ))}
            </tbody>
      ...
      <GlassCard>
        ...
        <ol className="mt-3 space-y-2 text-xs">
          {[
            ...
            <li key={i} className="flex items-center justify-between rounded-xl bg-background/40 p-2.5">
              <span><span className="font-semibold">{a.w}</span> {a.t}</span>
